...
def refresh_news(client:D1IngestClient,*,limit=20,apply=False):
    ...
    report={"targets":0,"selected":0,"written":0,"failures":[]};today=dt.date.today()
    for row in client.active_ipos(limit=limit,lookback_days=30):
        try:
            ...
            report["targets"]+=1;name=str(bundle["issue"]["name"]);sym=bundle["issue"].get("nse_symbol");best=None
            ...
            report["selected"]+=1;sc,title,link,pub,date,desc=best;published=None
            ...
            if apply:
                client.op({"op":"news_upsert","company_name":name,"nse_symbol":sym,"publisher":pub,"headline":title,"url":link,
                           "published_at":published,"snippet":desc,"selection_score":sc,"source":"rss","fetch_status":"ok","is_current":True});report["written"]+=1
...
def refresh_gmp(client:D1IngestClient,*,apply=False):
    ...
    try:data=scrape()
    except BaseException as exc:report["failures"].append({"stage":"scrape","error":f"{type(exc).__name__}:{exc}"});return report
    report["scraped"]=len(data)
    for d in data:
        try:
            row=client.resolve_identity(name_norm=_norm(d["company"]))
            if not row:report["unresolved"].append(d["company"]);continue
...
def main(argv=None):
    ap=argparse.ArgumentParser();ap.add_argument("--limit",type=int,default=20);ap.add_argument("--apply",action="store_true");a=ap.parse_args(argv)
    rep=run(limit=max(1,min(a.limit,50)),apply=a.apply);print("D1_STREET_GMP_SUMMARY="+json.dumps(rep,sort_keys=True,default=str));return 1 if rep["news"]["failures"] or rep["gmp"]["failures"] else 0
