...
async function getSignals(): Promise<TechnicalSignal[]> {
  const result = await sql`
    SELECT
      ...
      (
        (CASE WHEN monthly_ok THEN 1 ELSE 0 END) +
        (CASE WHEN weekly_ok THEN 1 ELSE 0 END) +
        (CASE WHEN weekly_ha_green_no_lower_shadow THEN 1 ELSE 0 END) +
        (CASE WHEN daily_nr7_recent THEN 1 ELSE 0 END) +
        (CASE WHEN daily_inside_bar_recent THEN 1 ELSE 0 END)
      )                                     AS criteria_met,
      (
        monthly_ok AND weekly_ok AND
        weekly_ha_green_no_lower_shadow AND
        daily_nr7_recent AND daily_inside_bar_recent
      )                                     AS all_criteria_met,
...
async function getStats() {
  const r = await sql`
    SELECT
      COUNT(*)                                                              AS total,
      COUNT(*) FILTER (WHERE monthly_ok AND weekly_ok
        AND weekly_ha_green_no_lower_shadow
        AND daily_nr7_recent AND daily_inside_bar_recent)                  AS strong,
  ...
  return (
    <MultibaggerClient
      ...
      stats={{
        ...
        latestDate:   stats.latest_date ? String(stats.latest_date) : "",
      }}
    />
  );
}
