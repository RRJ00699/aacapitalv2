...
export async function GET(req: Request) {
  ...
  const isWarm = url.searchParams.get("warm") === "1"
    && req.headers.get("x-aac-key") === process.env.ADMIN_JOB_KEY;
  // Normal page loads need a session; the pipeline's warm call uses the machine
  // key instead (rebuilds cache after a run, no human session available).
  if (!isWarm) {
  ...
  // Warm requests SKIP the cache read (they exist to refresh it) and fall
  // straight to the DB query below, then write KV on the way out.
  if (kv && !isWarm) {
    try {
