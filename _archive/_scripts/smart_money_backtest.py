#!/usr/bin/env python3
"""
smart_money_backtest.py — does INSTITUTIONAL ACCUMULATION predict forward returns?
...
TOPPCT = float(os.environ.get("SM_TOPPCT", "20"))
...
def main():
    ...
    for qd in sorted(sig["quarter_date"].unique()):
        ...
        if len(q) < 40:
            continue
        cut = np.percentile(q["delta"], 100 - TOPPCT)
        ...
        for hd in FWDS:
            ...
            for _, r in q.iterrows():
                ...
                if r["delta"] >= cut:
                    acc.append(fr)
            if len(acc) >= 5 and len(uni) >= 20:
                records.append({"qd": pd.Timestamp(qd), "year": pd.Timestamp(qd).year,
                                "hd": hd, "acc": np.array(acc), "uni": np.array(uni)})
    ...
    for hd in FWDS:
        ...
        if not rs:
            continue
        acc = np.concatenate([r["acc"] for r in rs])
        ...
        bw = (uni > 0).mean() * 100
        stat("UNIVERSE", uni)
        stat("ACCUMULATED", acc, bw)
        print("  " + "-" * 74)
        for yr in sorted({r["year"] for r in rs}):
            ay = np.concatenate([r["acc"] for r in rs if r["year"] == yr])
            uy = np.concatenate([r["uni"] for r in rs if r["year"] == yr])
            if len(ay) < 15:
                print(f"  {yr}: too few"); continue
            stat(f"  {yr} accum", ay, (uy > 0).mean() * 100)
    print("\n" + "=" * 80)
    print("Verdict: ACCUMULATED must beat UNIVERSE by >3pts win% with EDGE+ across the years")
    print("present, and a tolerable worst10%. NOTE: limited quarters = treat as indicative.")
