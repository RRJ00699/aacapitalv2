...
const contracts = {
  ...
  "journey:isin:INE000000001:v1": { isin: "INE000000001", sym: "EXAMPLE", rows: [{ date: "2026-08-04", open: 100, high: 110, low: 99, close: 108, volume: 1 }], generated_at: "2026-08-04T00:00:00Z" },
...
class FailingPointerKV extends MemoryKV {
  async put(k: string, v: string) {
    if (k.endsWith(":active") && v === "v2") throw new Error("simulated pointer failure");
    return super.put(k, v);
  }
}

test("pointer-update partial failure leaves prior version readable", async () => {
  const kv = new FailingPointerKV();
  await publishVersionedSnapshot(kv, "ipo:index:v3", { rows: [{ sym: "SAFE" }] }, "v1");
  await assert.rejects(() => publishVersionedSnapshot(kv, "ipo:index:v3", { rows: [{ sym: "NEW" }] }, "v2"));
  const hit = await readVersionedSnapshot(kv, "ipo:index:v3");
  assert.equal(JSON.parse(hit!.payload).rows[0].sym, "SAFE");
});
