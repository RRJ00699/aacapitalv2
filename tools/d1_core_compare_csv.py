...
def d1_query(config: Path, binding: str, remote: bool, sql: str) -> list[dict[str, Any]]:
    ...
    cmd = [npx_cmd(), "wrangler", "d1", "execute", binding, "--command", sql, "--json", target, "--config", str(config)]
...
def choose(d1v, neonv, matrixv):
    if d1v not in (None, ""):
        if neonv not in (None, "") and str(d1v) != str(neonv):
            return "CONFLICT_KEEP_D1", None
        if matrixv not in (None, "") and str(d1v) != str(matrixv):
            return "CONFLICT_KEEP_D1", None
        return "PRESENT", None
    if neonv not in (None, "") and matrixv not in (None, ""):
        if str(neonv) == str(matrixv):
            return "MISSING_D1_AGREE", str(neonv)
        return "MISSING_D1_SOURCE_CONFLICT", str(neonv)
    if neonv not in (None, ""):
        return "MISSING_D1_NEON", str(neonv)
    if matrixv not in (None, ""):
        return "MISSING_D1_MATRIX", str(matrixv)
...
def main() -> int:
    ...
    table_cache: dict[str, dict[Any, dict[str, Any]]] = {}
    for table, key_sql, key_fn in (
        ("ipo_issue", "ipo_id", lambda r: r.get("ipo_id")),
        ("company_profile", "ipo_id", lambda r: r.get("ipo_id")),
        ("anchor_summary", "ipo_id", lambda r: r.get("ipo_id")),
    ):
        rows = d1_query(config, args.binding, remote, f"SELECT * FROM {table}")
        table_cache[table] = {key_fn(r): r for r in rows}
    ...
    issue_neon_fields = {
        "open_date": "open_date", "close_date": "close_date", "allotment_date": "allotment_date",
        "listing_date": "listing_date", "band_lo_rs": "band_lo", "band_hi_rs": "band_hi",
        "issue_price_rs": "issue_price", "face_value_rs": "face_value", "lot_size_shares": "lot_size",
        "issue_size_cr": "issue_size_cr", "fresh_cr": "fresh_cr", "ofs_cr": "ofs_cr", "registrar_name": "registrar",
    }
    fin_neon_fields = {
        "revenue_cr": "revenue", "total_income_cr": "total_income", "ebitda_cr": "ebitda", "pat_cr": "pat",
        "net_worth_cr": "net_worth", "debt_cr": "total_debt", "assets_cr": "total_assets",
    }
    ...
    for base in args.ipomatrix:
        for path in sorted(base.rglob("*.json")):
            try:
                ...
                continue
            mid = json_path(payload, mapping.get("matrix_id"))
            name = json_path(payload, mapping.get("name"))
            isin = json_path(payload, mapping.get("isin"))
            ...
            ipo_id = (d1spine or neon or {}).get("id") or (neon or {}).get("ipo_id")
            if ipo_id is None:
                output.append({"matrix_id":mid,"ipo_id":"","company_name":name,"table":"ipo","row_key":"","field":"identity","d1_value":"","neon_value":"","matrix_value":str(isin or name),"status":"UNMATCHED_IDENTITY","recommended_value":"","approved":""})
                continue
            ...
            for section, table in (("ipo_issue","ipo_issue"),("company_profile","company_profile"),("anchor_summary","anchor_summary")):
                ...
                for field, rule in (mapping.get(section) or {}).items():
                    ...
                    nv = None
                    if section == "ipo_issue" and neon:
                        nv = norm_value(neon.get(issue_neon_fields.get(field)))
                    dv = norm_value(d1row.get(field))
                    status, recommended = choose(dv, nv, mv)
                    if status != "ABSENT_ALL":
                        output.append({"matrix_id":mid,"ipo_id":ipo_id,"company_name":name,"table":table,"row_key":str(ipo_id),"field":field,"d1_value":dv or "","neon_value":nv or "","matrix_value":mv or "","status":status,"recommended_value":recommended or "","approved":""})

            fin_spec = mapping.get("financial_statements") or {}
            rows = json_path(payload, fin_spec.get("rows")) or []
            if isinstance(rows, list):
                for item in rows:
                    if not isinstance(item, dict):
                        continue
                    fields = fin_spec.get("fields") or {}
                    period = norm_value(json_path(item, (fields.get("period") or {}).get("path")))
                    basis = norm_value(json_path(item, (fields.get("basis") or {}).get("path")))
                    if not period or not basis:
                        continue
                    key = (ipo_id, period, basis)
                    d1row = table_cache["financial_statements"].get(key, {})
                    nrow = fin_by_key.get(key, {})
                    for field, rule in fields.items():
                        if field in ("period","basis") or not isinstance(rule, dict) or "path" not in rule:
                            continue
                        mv = norm_value(json_path(item, rule["path"]), rule.get("unit"), rule.get("normalized_unit", rule.get("unit")))
                        nv = norm_value(nrow.get(fin_neon_fields.get(field)))
                        dv = norm_value(d1row.get(field))
                        status, recommended = choose(dv, nv, mv)
                        if status != "ABSENT_ALL":
                            output.append({"matrix_id":mid,"ipo_id":ipo_id,"company_name":name,"table":"financial_statements","row_key":json.dumps({"ipo_id":ipo_id,"period":period,"basis":basis},separators=(",",":")),"field":field,"d1_value":dv or "","neon_value":nv or "","matrix_value":mv or "","status":status,"recommended_value":recommended or "","approved":""})

    args.output.parent.mkdir(parents=True, exist_ok=True)
    fields = ["matrix_id","ipo_id","company_name","table","row_key","field","d1_value","neon_value","matrix_value","status","recommended_value","approved"]
    with args.output.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader(); writer.writerows(output)
    counts = defaultdict(int)
    for row in output: counts[row["status"]] += 1
    print(json.dumps({"rows":len(output),"status_counts":dict(sorted(counts.items())),"output":str(args.output)},sort_keys=True))
    return 0
