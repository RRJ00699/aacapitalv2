...
def run_wrangler(config: Path, binding: str, remote: bool, args: list[str]):
    ...
    subprocess.run([npx_cmd(),"wrangler","d1",*args,target,"--config",str(config)],cwd=ROOT,check=True,text=True,encoding="utf-8",errors="replace")
...
def statement(row: dict[str,str]) -> str:
    ...
    if table not in ALLOWED or field not in ALLOWED[table]:
        raise ValueError(f"field not approved for import: {table}.{field}")
    value = row.get("recommended_value")
    if value in (None, ""):
        raise ValueError(f"approved row has no recommended_value: {table}.{field}")

    if table == "financial_statements":
        key = json.loads(row["row_key"])
        ipo_id = int(key["ipo_id"])
        period = key["period"]
        basis = key["basis"]
        return (
            f"INSERT INTO financial_statements(ipo_id,period,basis,{field}) "
            f"VALUES({ipo_id},{sql_value(period)},{sql_value(basis)},{sql_value(value)}) "
            f"ON CONFLICT(ipo_id,period,basis) DO UPDATE SET {field}=COALESCE(financial_statements.{field},excluded.{field});"
        )

    ipo_id = int(row["ipo_id"])
    return (
        f"INSERT INTO {table}(ipo_id,{field}) VALUES({ipo_id},{sql_value(value)}) "
        f"ON CONFLICT(ipo_id) DO UPDATE SET {field}=COALESCE({table}.{field},excluded.{field});"
    )
...
def execute(config: Path, binding: str, remote: bool, statements: list[str], max_file_bytes: int):
    ...
    def flush():
        ...
        done+=len(pending);print(f"approved_core_fields: {done} / {len(statements)}")
        pending=[];size=24
...
def main() -> int:
    ...
    if unsafe:
        raise SystemExit(f"refusing {len(unsafe)} approved non-missing rows; importer never overwrites existing D1 facts")
    statements=[statement(r) for r in approved]
    execute(args.wrangler_config.resolve(),args.binding,remote,statements,args.max_file_bytes)
    print(json.dumps({"approved_rows":len(approved),"applied_rows":len(statements)},sort_keys=True))
    return 0
