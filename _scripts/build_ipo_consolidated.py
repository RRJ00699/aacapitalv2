...
MAP = [
  ...
  ("return_day1_open",        "i.return_day1_open"),
  ("return_day1_close",       "i.return_day1_close"),
  ("return_day1_vwap",        "i.return_day1_vwap"),
  ("return_day2",             "i.return_day2"),
  ("return_day7",             "i.return_day7"),
  ("return_day15",            "i.return_day15"),
  ("return_day30",            "i.return_day30"),
  ("return_day90",            "i.return_day90"),
  ("return_day180",           "i.return_day180"),
  ("return_day365",           "i.return_day365"),
...
def main():
    ...
    build = f"""
      ...
      LEFT JOIN ipo_issue_details        d ON UPPER(d.nse_symbol) = UPPER(i.nse_symbol)
      LEFT JOIN ipo_subscription_history s ON UPPER(s.nse_symbol) = UPPER(i.nse_symbol);
    """
    cur.execute(build)
    ...
    n = cur.fetchone()[0]
    cur.execute("SELECT count(nse_symbol), count(final_total), count(roe), count(lqi_final) FROM ipo_consolidated")
    nsym, nsub, nroe, nlqi = cur.fetchone()
    print(f"ipo_consolidated built: {n} rows, {len(MAP)} columns")
    print(f"  fill rates — nse_symbol {nsym}/{n}, subscription {nsub}/{n}, roe {nroe}/{n}, lqi_final {nlqi}/{n}")
    conn.close()
