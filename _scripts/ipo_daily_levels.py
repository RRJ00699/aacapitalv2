...
def write_db(s, replace=False):
    ...
    # daily appends use DO NOTHING (past days fixed). --replace recomputes ALL rows (e.g. method change).
    conflict = ("""ON CONFLICT (symbol,date) DO UPDATE SET
        t=EXCLUDED.t, close=EXCLUDED.close, floor=EXCLUDED.floor, ceiling=EXCLUDED.ceiling,
        poc=EXCLUDED.poc, broke_floor=EXCLUDED.broke_floor, broke_ceiling=EXCLUDED.broke_ceiling,
        cushion=EXCLUDED.cushion""" if replace else "ON CONFLICT (symbol,date) DO NOTHING")
    execute_values(cur,f"""INSERT INTO ipo_daily_levels
        (symbol,date,t,close,floor,ceiling,poc,broke_floor,broke_ceiling,cushion) VALUES %s
        {conflict}""",rows)
    conn.commit(); conn.close()
    print(f"✓ {'replaced' if replace else 'upserted'} {len(rows)} rows in ipo_daily_levels"
          f"{'' if replace else ' (golden-rule: past days untouched)'}")
...
if __name__=="__main__":
    ...
    ap.add_argument("--replace",action="store_true",help="overwrite existing rows (use after a method change)")
    a=ap.parse_args()
    meta,cand = load_db() if a.from_db else load_csv(a.meta,a.candles)
    s=run(meta,cand)
    print(f"daily level rows: {len(s)} across {s['sym'].nunique()} IPOs")
    if a.write_db: write_db(s, replace=a.replace)
    else:
